...
expect_rejection "$OUT/mutant_zero.sio" mutant_zero 'FAIL independent basis bridge'

# Mutation of the recursive sign convention must disagree with the independent
# pair formula, which does not call basis_sign or cd_mul.
sed 's/let sign = if j - h == 0 { -1 } else { 1 }/let sign = if j - h == 0 { 1 } else { -1 }/' \
  "$HERE/rcda_core.sio" > "$OUT/mutant_sign.sio"
expect_rejection "$OUT/mutant_sign.sio" mutant_sign 'FAIL independent basis bridge'
