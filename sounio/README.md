...
- `verification/`: positive run, four mutation checks, two domain-refusal checks,
...
An independent recursive pair-product implementation also checks all 341 basis
pairs across the supported dimensions and 16 dense signed pairs. It never calls
the sign/XOR multiplication.
...
The `[V]` prefix identifies execution of finite instances associated with
...
convention and independently checked named witnesses. There is now a universal
refinement theorem for an explicit mathematical model of the sixteen-coordinate
loop, plus index and accumulator bounds: see [PROOF_BRIDGE.md](../PROOF_BRIDGE.md).
The source-to-model translation and compiler correctness are not proved.

## The gate must be able to fail

The gate compiles six altered Sounio sources and requires controlled runtime rejection:

1. Replace every product contribution by zero — rejected by the independent
   pair-product comparison.
2. Invert the high/high sign branch — rejected by the independent comparison.
3. Replace `e15` by `e14` in the complementary factor — rejected by the annihilation
   assertion.
4. Remove the quadratic curvature term — rejected by the curvature equality.
5. Supply a coefficient of 1,000,001 — rejected by the exact-domain guard.
6. Multiply dimension-eight and dimension-sixteen inputs — rejected by the
   dimension guard.
